...
apiRouter.get('/cash-sessions/active', authenticateToken, async (req, res) => {
    try {
        const { rows } = await pool.query('SELECT * FROM cash_sessions WHERE is_active = true LIMIT 1');
        res.json(rows[0] || null);
    } catch (err) {
        res.status(500).json({ message: "Error checking for active session", error: err.message });
    }
});
apiRouter.post('/cash-sessions/start', authenticateToken, async (req, res) => {
    const { start_amount } = req.body;
    const { id: employee_id, name: employee_name } = req.user;
    if (start_amount === undefined || start_amount < 0) {
        return res.status(400).json({ message: "A valid start amount is required." });
    }
    ...
    try {
        ...
        res.status(500).json({ message: "Error starting new session", error: err.message });
    } finally {
        client.release();
    }
});
apiRouter.post('/cash-movements', authenticateToken, async (req, res) => {
    const { type, amount, reason, adminRut, adminPassword } = req.body;
    const requestingUser = req.user;

    if(!type || !amount || !reason || !adminRut || !adminPassword) {
    ...
    try {
        ...
        const isPasswordMatch = await bcrypt.compare(adminPassword, admin.password_hash);
        if (!isPasswordMatch) {
        ...
        let newBalance;
        if (type === 'ADD') {
            newBalance = parseFloat(session.current_balance) + parseFloat(amount);
        } else if (type === 'REMOVE') {
            newBalance = parseFloat(session.current_balance) - parseFloat(amount);
            if (newBalance < 0) {
                throw new Error('El retiro no puede dejar la caja con saldo negativo.');
            }
        } else {
        ...
        const actionType = type === 'ADD' ? 'CASH_ADD' : 'CASH_REMOVE';
        const details = `${type === 'ADD' ? 'Agregó' : 'Retiró'} $${amount}. Motivo: ${reason}. Aprobado por: ${admin.first_name}.`;
...
apiRouter.post('/sales', authenticateToken, async (req, res) => {
    const { items, total_amount, payment_method, adminRut, adminPassword } = req.body;
    const client = await pool.connect();
...
apiRouter.put('/products/:id', authenticateToken, async (req, res) => {
    const { id } = req.params;
    const { name, code, category, cost_price, stock, supplier_id, recalculate_price, fecha_vencimiento } = req.body;
    try {
...
apiRouter.get('/sales', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        if (startDate && endDate) {
            filterClause = ` WHERE created_at BETWEEN $1 AND $2`;
        ...
        const salesWithItems = await Promise.all(salesResult.rows.map(async (sale) => {
            const itemsResult = await pool.query(`SELECT si.*, p.name as product_name FROM sale_items si JOIN products p ON si.product_id = p.id WHERE si.sale_id = $1`, [sale.id]);
            return { ...sale, type: 'SALE', items: itemsResult.rows };
        }));
        ...
        const logsFormatted = logsResult.rows.map(log => ({ ...log, type: 'LOG' }));
...
apiRouter.get('/suppliers', authenticateToken, async (req, res) => {
    try { res.json((await pool.query('SELECT * FROM suppliers ORDER BY name')).rows); }
